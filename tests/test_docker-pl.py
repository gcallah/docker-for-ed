...
class TestLanguages(unittest.TestCase):
    ...
    def test_cpp(self):
        ...
        response = client.containers.run("pl", command="which gdb", remove=True, name="plcontainer").decode("utf-8")
        self.assertEqual(response, "/usr/bin/gdb\n", "gdb missing!")

    def test_lisp(self):
        print("[INFO] Checking Lisp installation")
        response = client.containers.run("pl", command="which sbcl", remove=True, name="plcontainer").decode("utf-8")
